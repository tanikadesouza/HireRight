...
export function NoteForm({ sessionId, onNoteAdded }: NoteFormProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (noteText.length > MAX_NOTE_CHARS) {
      setError(`Note too long (max ${MAX_NOTE_CHARS.toLocaleString()} characters)`);
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <textarea
        ...
        disabled={isPending}
      />
      <p className={`text-xs text-right ${charCount > MAX_NOTE_CHARS * 0.9 ? "text-amber-600" : "text-gray-400"}`}>
        {charCount.toLocaleString()} / {MAX_NOTE_CHARS.toLocaleString()}
      </p>
